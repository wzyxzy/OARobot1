...
import com.zgty.oarobot.bluetools.BuleToolsOpenDoor;
...
//                    mSpeech.speak("请面对摄像头保持微笑", TextToSpeech.QUEUE_FLUSH, null);
//                    robotSpeek("请面对摄像头保持微笑", 0, 0, 0);
                    break;
            }
...
//                            robotSpeek("前面已有联系任务，您还需等待" + time_second + "秒", 0, 15);
                    ...
                    } else if ((!text.contains("不")) && text.contains("联系")) {
                        if (canConnect) {
                            robotSpeek(String.format(speekDaoUtils.querySpeekingText("connectForYou"), username), 2, 10, 0);
//                            userid1 = userid;
                        } else {
...
//                            robotSpeek("前面已有联系任务，您还需等待" + time_second + "秒", 0, 15);
                    ...
                    } else if ((!text.contains("不")) && text.contains("好")) {
                        if (canConnect) {
                            robotSpeek(String.format(speekDaoUtils.querySpeekingText("connectForYou"), username), 2, 10, 0);
//                            userid1 = userid;
                        } else {
...
//                            weiXinUtils.SendText("前台有人找您，他的照片发给您");
    ...
    private void successConnect() {
        ...
        handler1.postDelayed(runnable, 1000);//每两秒执行一次runn
        // able.
//        new Handler().postDelayed(new Runnable() {
//            @Override
//            public void run() {
...
//        }, 60000);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        initView();
        new BuleToolsOpenDoor().initConnectData(); //初识化蓝牙连接并设置开门持续时间为5秒
        createCameraSource();
        requestPermissions();
...
//        int var6 = var4.getDefaultDisplay().getRotation();
    ...
    private void initInfo() {
        ...
        if (speakings == null || speakings.size() == 0) {
            ...
            Speaking speaking8 = new Speaking("cannotRecognise", "陌生人", "我还不认识您，请问您找谁?请在屏幕提示聆听字样时开始说话");
            Speaking speaking9 = new Speaking("cannotHearingClear", "没有听清", "我没有听清您说的话，请问您找谁？请在屏幕提示聆听字样时开始说话");
...
//            }
            ...
            new BuleToolsOpenDoor().openDoorOperation();//蓝牙开门.
...
//            handler.sendEmptyMessage(0);
        ...
        public void onUpdate(FaceDetector.Detections<Face> detectionResults, Face face) {
            ...
            if (isFirst && lastSmile - firstSmile >= 0.001) {
